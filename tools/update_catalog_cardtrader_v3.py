...
MAX_MARKETPLACE_CALLS = int(os.getenv("MAX_MARKETPLACE_CALLS", "200"))
...
MAX_CALLS_PER_ENTRY = int(os.getenv("MAX_CALLS_PER_ENTRY", "30"))
EARLY_VALID_PER_ENTRY = int(os.getenv("EARLY_VALID_PER_ENTRY", "12"))


def request_json(url, token=False):
    headers = {"Accept": "application/json", "User-Agent": "PokeBinder-CatalogUpdater/9.0"}
    if token:
        if not TOKEN:
...
def build_index():
    ...
    pokemon = next((g for g in games if "pokemon" in {normalize(g.get("name")), normalize(g.get("display_name"))}), None)
    if not pokemon or pokemon.get("id") is None:
        raise RuntimeError("Pokémon não localizado em /games")
    game_id = int(pokemon["id"])

    categories = records(api("/categories", {"game_id": game_id}), "/categories")
    single_ids = {int(c["id"]) for c in categories if c.get("id") is not None and "single" in normalize(c.get("name") or c.get("display_name"))}
    if not single_ids:
        raise RuntimeError("Categoria Singles não localizada")

    expansions = [e for e in records(api("/expansions"), "/expansions") if int(e.get("game_id", -1) or -1) == game_id]
    expansion_map = {int(e["id"]): e for e in expansions if e.get("id") is not None}
    names = {}
    for pos, expansion in enumerate(expansions, 1):
        try:
            blueprints = records(api("/blueprints/export", {"expansion_id": expansion["id"]}), "/blueprints/export")
        ...
        for bp in blueprints:
            ...
            item = {"id": int(bp["id"]), "name": name, "expansionId": expansion_id, "expansion": ex.get("name"), "expansionCode": ex.get("code")}
            names.setdefault(normalize(name), []).append(item)
        if pos % 100 == 0:
            print(f"{pos}/{len(expansions)} expansões indexadas")
        time.sleep(INDEX_REQUEST_DELAY)

    payload = {"version": 2, "generatedAt": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "generatedAtEpoch": int(time.time()), "gameId": game_id, "singleCategoryIds": sorted(single_ids), "names": names}
    save(INDEX, payload)
...
def candidates(target, index):
    ...
    if len(ordered) <= 8:
        return ordered
    # Intercala impressões antigas e recentes para aumentar a chance de achar cartas valiosas cedo.
...
def reference_for(bp):
    ...
    return {"name": bp["name"], "code": bp.get("expansionCode") or str(bp["id"]), "collection": bp.get("expansion"), "value": f"{symbol} {reference / 100:,.2f}", "source": SOURCE, "url": MANUAL_URL, "blueprintId": bp["id"], "referenceCents": reference, "currency": currency, "priceMethod": "Mediana de até 15 ofertas Near Mint mais baratas"}
...
def main():
    ...
    for number in numbers:
        ...
        for target in targets:
            ...
            entry_calls = 0
            entry_valid = 0
            while cursor < len(pool) and calls < MAX_MARKETPLACE_CALLS and entry_calls < MAX_CALLS_PER_ENTRY:
                bp = pool[cursor]
                cursor += 1
                calls += 1
                entry_calls += 1
                try:
                    ...
                    if exc.code == 429:
                        cursor -= 1
                        print("Rate limit 429; preservando cursor e encerrando lote")
                        calls = MAX_MARKETPLACE_CALLS
                        break
                ...
                if entry_valid >= EARLY_VALID_PER_ENTRY:
                    early_stops += 1
                    print(f"  Parada antecipada: {entry_valid} referências válidas em {entry_calls} consultas; cursor preservado em {cursor}/{len(pool)}")
                    break
            ...
            entry["lastEntryCalls"] = entry_calls
            entry["lastEntryValidObserved"] = entry_valid
    ...
    catalog.update({"version": 9, "updatedAt": time.strftime("%Y-%m-%d"), "priceSource": SOURCE, "sources": [SOURCE], "currencyPolicy": "Moeda retornada pela CardTrader", "priceMethod": "Mediana de até 15 ofertas Near Mint mais baratas por blueprint", "catalogScope": "Catálogo acelerado incremental por espécie e forma usando a API oficial CardTrader", "note": "Até 3 cartas por entrada. Primeira passada prioriza cobertura rápida; cursores incompletos são retomados nas passadas seguintes. Formas Shiny não recebem cartas normais automaticamente.", "stats": {"entries": len(entries), "entriesWithCards": filled, "cards": total_cards, "entriesIncomplete": incomplete}})
    ...
    for obsolete in ("stoppedByLigaBlock", "blockedHttpStatus", "blockedEntryName", "stoppedByRateLimit", "resetReason", "collectorMode", "requestDelaySeconds", "lastBatchEntriesWithNewCards", "lastBatchCardsAdded"):
        progress.pop(obsolete, None)
    progress.update({"version": 5, "lastRunAt": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "lastBatchSpecies": processed, "lastBatchMarketplaceCalls": calls, "lastBatchCardsObserved": observed, "lastBatchEarlyStops": early_stops, "entriesIncomplete": incomplete, "totalRuns": int(progress.get("totalRuns", 0)) + 1, "sourcePolicy": "CardTrader-only", "collectorMode": "accelerated-fair-share", "parserVersion": "9.0-accelerated", "batchSize": BATCH_SIZE, "maxMarketplaceCalls": MAX_MARKETPLACE_CALLS, "maxCallsPerEntry": MAX_CALLS_PER_ENTRY, "earlyValidPerEntry": EARLY_VALID_PER_ENTRY, "marketplaceDelaySeconds": MARKETPLACE_DELAY})

    save(CATALOG, catalog)
    save(PROGRESS, progress)
    print(f"CardTrader acelerado: {filled} entradas / {total_cards} cartas; incompletas={incomplete}; calls={calls}; espécies={processed}; early_stops={early_stops}")
